...
public class MainActivity extends Activity {
    ...
    private LinearLayout makeBrowserPanel(){ LinearLayout p=new LinearLayout(this); p.setOrientation(LinearLayout.VERTICAL); p.setPadding(dp(8),0,dp(8),dp(8)); TextView h=label("صفحه رجا — هنگام پیدا شدن بلیت خودکار باز می‌شود"); h.setGravity(Gravity.CENTER); h.setTextColor(CYAN); p.addView(h,new LinearLayout.LayoutParams(-1,dp(38))); Button reload=new Button(this); reload.setText("رفرش صفحه"); reload.setOnClickListener(v->{ if(running)stopBot("توقف برای رفرش دستی؛ برای ادامه شروع ربات را بزن"); lastFinishedAt=0; webView.reload(); }); p.addView(reload,new LinearLayout.LayoutParams(-1,dp(44))); webView=new WebView(this); webView.setBackgroundColor(Color.WHITE); p.addView(webView,new LinearLayout.LayoutParams(-1,0,1)); return p; }
    ...
    private void stopBot(String why){ running=false; formInProgress=false; loginInProgress=false; sessionGeneration++; if(sessionDeadline!=null){handler.removeCallbacks(sessionDeadline);sessionDeadline=null;} webView.evaluateJavascript("if(window.__masoudSessionCancel)window.__masoudSessionCancel();window.__masoudFormCancelled=true;",null); actionState=2; if(monitorRunnable!=null) handler.removeCallbacks(monitorRunnable); updateActionButtons(); status.setText("■ "+why); status.setTextColor(RED); log(why); }
    ...
    private void injectSearchFormV11(){
        ...
        String js="(async function(){window.__masoudFormCancelled=false;const cfg="
            +"{\"token\":"+token+",\"origin\":"+q(origin.getText().toString().trim())+",\"destination\":"+q(destination.getText().toString().trim())+",\"date\":"+q(travelDate.getText().toString().trim())+",\"adults\":"+adultCount+",\"children\":"+childCount+",\"passengerType\":"+q(passengerType)+",\"coupe\":"+(coupe.isChecked()?"true":"false")+"};"
            +"\nconst norm=s=>String(s||'').replace(/[\\u200c\\u200e\\u200f]/g,' ').replace(/ي/g,'ی').replace(/ك/g,'ک').replace(/\\s+/g,' ').trim();\nconst latin=s=>String(s||'').replace(/[۰-۹]/g,d=>'۰۱۲۳۴۵۶۷۸۹'.indexOf(d)).replace(/[٠-٩]/g,d=>'٠١٢٣٤٥٦٧٨٩'.indexOf(d));\nconst txt=e=>norm(e&&(e.innerText||e.textContent||e.value||e.getAttribute&&e.getAttribute('aria-label')||''));\nconst vis=e=>!!(e&&e.getClientRects&&e.getClientRects().length&&getComputedStyle(e).visibility!=='hidden'&&getComputedStyle(e).display!=='none');\nconst all=s=>[...document.querySelectorAll(s)].filter(vis);\nconst sleep=ms=>new Promise(r=>setTimeout(r,ms)).then(()=>{if(window.__masoudFormCancelled)throw new Error('cancelled');});\nconst wait=async(fn,ms=12000)=>{const end=Date.now()+ms;while(!window.__masoudFormCancelled&&Date.now()<end){let v=null;try{v=fn();}catch(e){}if(v)return v;await sleep(120);}return null;};\nconst fire=e=>{for(const n of ['input','change','blur'])e.dispatchEvent(new Event(n,{bubbles:true}));};\nconst nativeSet=(e,v)=>{try{const p=e.tagName==='TEXTAREA'?HTMLTextAreaElement.prototype:HTMLInputElement.prototype;const d=Object.getOwnPropertyDescriptor(p,'value');if(d&&d.set)d.set.call(e,v);else e.value=v;}catch(_){e.value=v;}fire(e);};\nconst fail=m=>{MasoudBridge.moduleFailed(cfg.token,m);return false;};\nconst meta=e=>norm((e.getAttribute?.('placeholder')||'')+' '+(e.getAttribute?.('aria-label')||'')+' '+(e.getAttribute?.('name')||'')+' '+(e.getAttribute?.('id')||'')+' '+(e.parentElement?.innerText||'')).toLowerCase();\nconst clickable=e=>e&&(e.closest?.('button,a,[role=button],[role=option],.ng-option,li')||e);\n\nasync function chooseOption(value){\n  const wanted=norm(value);\n  const option=await wait(()=>all('ng-dropdown-panel .ng-option,[role=option],mat-option,.dropdown-menu li,.dropdown-menu a,li').filter(e=>txt(e)).sort((a,b)=>txt(a).length-txt(b).length).find(e=>txt(e)===wanted)||all('ng-dropdown-panel .ng-option,[role=option],mat-option,.dropdown-menu li,.dropdown-menu a,li').find(e=>txt(e).includes(wanted)),8000);\n  if(!option)return false;\n  clickable(option).click();await sleep(250);return true;\n}\nfunction inputFor(kind){\n  const keys=kind==='from'?['مبدا','مبدأ','origin','from','departure']:['مقصد','destination','to','arrival'];\n  let comp=all('app-stations,ng-select,[role=combobox]').find(e=>{const m=meta(e);return keys.some(k=>m.includes(norm(k).toLowerCase()));});\n  if(comp){const i=comp.querySelector('input:not([type=hidden]),textarea');if(i&&vis(i))return i;}\n  const inputs=all('input:not([type=hidden]),textarea');\n  let best=null,bestScore=-1;\n  for(const i of inputs){const m=meta(i);let score=0;for(const k of keys)if(m.includes(norm(k).toLowerCase()))score+=10;if(/date|تاریخ|phone|mobile|رمز|password/.test(m))score-=12;if(score>bestScore){best=i;bestScore=score;}}\n  return bestScore>0?best:null;\n}\nasync function station(kind,city){\n  for(let attempt=0;attempt<3;attempt++){\n    let inp=await wait(()=>inputFor(kind),7000);\n    if(!inp)return false;\n    inp.scrollIntoView({block:'center'});inp.focus();\n    let container=inp.closest('app-stations,ng-select,[role=combobox],.ng-select')||inp.parentElement;\n    let clear=container&&container.querySelector('.ng-clear-wrapper,.ng-clear,[aria-label*=پاک],[title*=پاک]');\n    if(clear)clickable(clear).click();\n    nativeSet(inp,'');await sleep(100);nativeSet(inp,city);\n    inp.dispatchEvent(new KeyboardEvent('keyup',{key:'ا',bubbles:true}));\n    await sleep(250);\n    if(await chooseOption(city)){\n      const ok=await wait(()=>norm(inp.value).includes(norm(city))||norm(container?.innerText).includes(norm(city)),3500);\n      if(ok)return true;\n    }\n    await sleep(300);\n  }\n  return false;\n}\nfunction dateNums(v){return latin(v).split(/[^0-9]+/).filter(Boolean).map(x=>Number(x)).join('-');}\nfunction dateInput(){\n  const candidates=all('app-datepicker-single input,input:not([type=hidden]),textarea');\n  let best=null,bestScore=-1;\n  for(const i of candidates){const m=meta(i);let score=0;if(/تاریخ|date|datepicker|oneway/.test(m))score+=12;if(/مبدا|مقصد|origin|destination|phone|password/.test(m))score-=15;if(score>bestScore){best=i;bestScore=score;}}\n  return bestScore>0?best:null;\n}\nasync function setDate(){\n  const target=cfg.date.replace(/-/g,'/');\n  const parts=target.split('/').map(Number);if(parts.length!==3||parts.some(x=>!x))return false;\n  let inp=await wait(dateInput,7000);if(!inp)return false;\n  inp.scrollIntoView({block:'center'});\n  if(!inp.readOnly&&!inp.disabled){\n    inp.focus();nativeSet(inp,target);inp.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}));inp.dispatchEvent(new KeyboardEvent('keyup',{key:'Enter',bubbles:true}));\n    if(await wait(()=>dateNums(inp.value)===parts.join('-'),2200))return true;\n  }\n  const picker=inp.closest('app-datepicker-single,.datepicker,.date-picker')||document;\n  let opener=picker.querySelector('button.icon-calendar,[aria-label*=تقویم],[title*=تقویم],button')||inp;\n  clickable(opener).click();await sleep(350);\n  const months=['','فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'];\n  for(const sel of all('select')){\n    const opts=[...sel.options], y=opts.find(o=>latin(txt(o)).includes(String(parts[0]))), m=opts.find(o=>txt(o).includes(months[parts[1]])||latin(txt(o))===String(parts[1]));\n    if(y){sel.value=y.value;fire(sel);await sleep(180);}\n    else if(m){sel.value=m.value;fire(sel);await sleep(180);}\n  }\n  const headerCandidates=all('button,[role=button],li,span,div').filter(e=>e.children.length<6);\n  const yNode=headerCandidates.find(e=>latin(txt(e))===String(parts[0]));\n  if(yNode&&clickable(yNode)){clickable(yNode).click();await sleep(150);}\n  const mNode=all('button,[role=button],li,span,div').find(e=>txt(e)===months[parts[1]]);\n  if(mNode&&clickable(mNode)){clickable(mNode).click();await sleep(150);}\n  const d=String(parts[2]),fa=d.replace(/[0-9]/g,x=>'۰۱۲۳۴۵۶۷۸۹'[Number(x)]);\n  const fullA=parts.join('/'),fullB=toFaDate(parts);\n  let cells=all('button,td,[role=gridcell]').filter(e=>{const c=String(e.className||'').toLowerCase(),a=norm((e.getAttribute('aria-label')||'')+' '+(e.getAttribute('title')||'')+' '+(e.getAttribute('data-date')||''));return !e.disabled&&!c.includes('disabled')&&!c.includes('muted')&&!c.includes('outside')&&(txt(e)===d||txt(e)===fa||a.includes(fullA)||a.includes(fullB));});\n  if(!cells.length)cells=all('button,td,[role=gridcell],span').filter(e=>{const c=String(e.className||'').toLowerCase();return !c.includes('disabled')&&!c.includes('muted')&&!c.includes('outside')&&(txt(e)===d||txt(e)===fa);});\n  if(!cells.length)return false;\n  const month=months[parts[1]];\n  let day=cells.find(e=>norm(e.closest('table,.calendar,.datepicker,.ngb-dp-month,.mat-calendar')?.innerText||'').includes(month))||cells[0];\n  clickable(day).click();await sleep(350);\n  inp=dateInput()||inp;\n  return !!(await wait(()=>dateNums(inp.value)===parts.join('-')||dateNums(inp.getAttribute('value'))===parts.join('-'),3500));\n}\nfunction toFaDate(p){return p.map(n=>String(n).replace(/[0-9]/g,d=>'۰۱۲۳۴۵۶۷۸۹'[Number(d)])).join('/');}\nfunction passengerButton(){\n  return document.querySelector('#dropdownPassenger')||all('button,[role=button],a,div').filter(e=>/مسافر/.test(txt(e))).sort((a,b)=>txt(a).length-txt(b).length)[0];\n}\nfunction numberFrom(s){const m=latin(s).match(/(?:^|\\D)([0-9])(?:\\D|$)/);return m?Number(m[1]):null;}\nasync function setCount(labels,want,def){\n  let label=await wait(()=>all('div,li,span,label,p').filter(e=>labels.some(x=>txt(e).includes(x))).sort((a,b)=>txt(a).length-txt(b).length)[0],6000);\n  if(!label)return false;\n  let row=label;\n  for(let i=0;i<7&&row&&row!==document.body;i++,row=row.parentElement){\n    if(row.querySelectorAll('button,[role=button]').length>=2||row.querySelector('input[type=number],[role=spinbutton]'))break;\n  }\n  if(!row)return false;\n  const spin=row.querySelector('input[type=number],[role=spinbutton]');\n  if(spin&&vis(spin)){nativeSet(spin,String(want));return !!(await wait(()=>Number(latin(spin.value))===want,1800));}\n  const read=()=>{for(const e of allWithin(row,'input,span,div,strong,b')){let n=numberFrom(e.value||txt(e));if(n!==null&&n>=0&&n<=9)return n;}return def;};\n  const buttons=allWithin(row,'button,[role=button]');\n  let plus=buttons.find(e=>/(\\+|افزایش|plus|add)/i.test(txt(e)+' '+(e.getAttribute('aria-label')||'')+' '+(e.getAttribute('title')||'')+' '+String(e.className||'')));\n  let minus=buttons.find(e=>/(−|-|کاهش|minus|remove)/i.test(txt(e)+' '+(e.getAttribute('aria-label')||'')+' '+(e.getAttribute('title')||'')+' '+String(e.className||'')));\n  if(!plus&&buttons.length>=2)plus=buttons[buttons.length-1];\n  if(!minus&&buttons.length>=2)minus=buttons[0];\n  for(let tries=0;tries<14;tries++){\n    const cur=read();if(cur===want)return true;\n    const b=cur<want?plus:minus;if(!b||b.disabled)return false;\n    clickable(b).click();if(!await wait(()=>read()!==cur,1600))return false;await sleep(100);\n  }\n  return read()===want;\n}\nfunction allWithin(root,sel){return [...root.querySelectorAll(sel)].filter(vis);}\nasync function setPassengerCounts(){\n  const pb=await wait(passengerButton,7000);if(!pb)return false;\n  clickable(pb).click();await sleep(450);\n  const a=await setCount(['بزرگسال'],cfg.adults,1);\n  let c=await setCount(['کودک','خردسال'],cfg.children,0);\n  if(!a||!c)return false;\n  const confirm=all('button,[role=button],a').find(e=>/^(تأیید|تایید|اعمال|بستن)$/.test(txt(e)));\n  if(confirm)clickable(confirm).click();else clickable(pb).click();\n  await sleep(300);return true;\n}\nasync function setPassengerType(){\n  const wanted=cfg.passengerType==='men'?'ویژه برادران':cfg.passengerType==='women'?'ویژه خواهران':'مسافران عادی';\n  const names=['مسافران عادی','مسافرین عادی','ویژه برادران','ویژه خواهران'];\n  let native=all('select').find(e=>[...e.options].filter(o=>names.includes(txt(o))).length>=2);\n  if(native){let o=[...native.options].find(o=>norm(txt(o)).replace('مسافرین','مسافران')===wanted);if(!o)return false;native.value=o.value;fire(native);return true;}\n  let current=all('ng-select,.ng-select-container,[role=combobox],button,a,div,span').filter(e=>names.includes(txt(e))).sort((a,b)=>txt(a).length-txt(b).length)[0];\n  if(!current)return false;\n  if(norm(txt(current)).replace('مسافرین','مسافران')===wanted)return true;\n  clickable(current).click();await sleep(250);\n  const choice=await wait(()=>all('ng-dropdown-panel .ng-option,[role=option],li,a,button,span').find(e=>norm(txt(e)).replace('مسافرین','مسافران')===wanted),5000);\n  if(!choice)return false;clickable(choice).click();await sleep(220);return true;\n}\nasync function setCoupe(){\n  if(!cfg.coupe)return true;\n  let label=all('label,div,span,p').filter(e=>txt(e).includes('کوپه دربست')).sort((a,b)=>txt(a).length-txt(b).length)[0];\n  if(!label)return false;\n  let box=label.closest('label,div')||label.parentElement;let cb=box&&box.querySelector('input[type=checkbox]');\n  if(cb){if(!cb.checked)clickable(cb).click();return true;}\n  clickable(label).click();return true;\n}\nasync function pressSearch(){\n  const b=await wait(()=>all('button,[role=button],input[type=submit],a').filter(e=>/جستجو/.test(txt(e))).sort((a,b)=>txt(a).length-txt(b).length)[0],7000);\n  if(!b)return false;\n  b.scrollIntoView({block:'center'});clickable(b).click();return true;\n}\n\ntry{\n  let change=all('button,a,[role=button],div,span').find(e=>txt(e)==='تغییر جستجو');if(change){clickable(change).click();await sleep(300);}\n  if(!await station('from',cfg.origin)){fail('مبدا در رجا تنظیم نشد');return;}MasoudBridge.log('W21: مبدا تأیید شد: '+cfg.origin);\n  if(!await station('to',cfg.destination)){fail('مقصد در رجا تنظیم نشد');return;}MasoudBridge.log('W21: مقصد تأیید شد: '+cfg.destination);\n  if(!await setDate()){fail('تاریخ در رجا تنظیم یا تأیید نشد');return;}MasoudBridge.log('W21: تاریخ تأیید شد: '+cfg.date);\n  if(!await setPassengerCounts()){fail('تعداد بزرگسال یا کودک در رجا تنظیم نشد');return;}MasoudBridge.log('W21: تعداد مسافران تأیید شد');\n  if(!await setPassengerType()){fail('نوع مسافر در رجا تنظیم نشد');return;}MasoudBridge.log('W21: نوع مسافر تأیید شد');\n  if(!await setCoupe()){fail('کوپه دربست در رجا پیدا نشد');return;}\n  if(!await pressSearch()){fail('دکمه جستجوی رجا پیدا نشد');return;}\n  MasoudBridge.searchReady(cfg.token);\n}catch(e){\n  if(!window.__masoudFormCancelled)MasoudBridge.moduleFailed(cfg.token,'خطای W21 در تنظیم فرم: '+(e&&e.message?e.message:e));\n}\n})();";
        webView.evaluateJavascript(js,null);
    }
