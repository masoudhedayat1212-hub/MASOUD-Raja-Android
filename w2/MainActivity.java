...
public class MainActivity extends Activity {
    ...
    private Button button(String text, int color) {
        Button b = new Button(this); b.setText(text); b.setTextColor(Color.WHITE); b.setTextSize(15); b.setAllCaps(false); b.setBackground(bg(color,14,color,0)); b.setMinHeight(dp(48)); b.setPadding(dp(8),0,dp(8),0); return b;
    }
    ...
    private LinearLayout makeRunPanel(){
        ...
        startBtn.setOnClickListener(v->startBot()); stopBtn.setOnClickListener(v->stopBot("توقف توسط کاربر")); priceMode.setOnCheckedChangeListener((b,checked)->{ trainNumber.setEnabled(!checked); minPrice.setEnabled(checked); maxPrice.setEnabled(checked); }); minPrice.setEnabled(false); maxPrice.setEnabled(false);
    ...
    private LinearLayout makePassengerStepper(String text,boolean adult){
        ...
        Button minus=button("−",PANEL_2); Button plus=button("+",BLUE); EditText val=field(""); val.setGravity(Gravity.CENTER); val.setInputType(InputType.TYPE_CLASS_NUMBER); val.setText(adult?"1":"0"); val.setFocusable(false); if(adult) adults=val; else children=val;
    ...
    private void startBot(){
        ...
        saveAccount(); running=true; loggedIn=false; searchSubmitted=false; reserved=false; alarmPlayed=false; startBtn.setEnabled(false); stopBtn.setEnabled(true); status.setText("● در حال شروع..."); logView.setText(""); log("شروع اجرا | "+origin.getText()+" ← "+destination.getText()+" | "+travelDate.getText());
