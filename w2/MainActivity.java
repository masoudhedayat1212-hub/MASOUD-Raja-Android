...
public class MainActivity extends Activity {
    ...
    private void injectLogout(){
        ...
        String js="(async function(){const sleep=ms=>new Promise(r=>setTimeout(r,ms));const t=s=>(s||'').replace(/\\s+/g,' ').trim();const vis=x=>x&&x.offsetParent!==null;try{let all=[...document.querySelectorAll('button,a,[role=button],span,div')].filter(vis);let account=all.filter(x=>/حساب کاربری|پروفایل|کاربری/.test(t(x.innerText))).sort((a,b)=>t(a.innerText).length-t(b.innerText).length)[0];if(!account){MasoudBridge.log('نشست فعالی برای خروج دیده نشد؛ ورود ادامه دارد');MasoudBridge.logoutDone();return;}account.click();let out=null;for(let i=0;i<20&&!out;i++){await sleep(50);let els=[...document.querySelectorAll('button,a,[role=button],span,div')].filter(vis);out=els.find(x=>/^خروج$/.test(t(x.innerText)))||els.find(x=>/خروج از حساب/.test(t(x.innerText)));}if(out){out.click();await sleep(150);MasoudBridge.log('فرمان خروج در رجا تأیید شد');}else MasoudBridge.log('دکمه خروج پیدا نشد؛ ورود مجدد ادامه دارد');MasoudBridge.logoutDone();}catch(e){MasoudBridge.log('خطای خروج: '+e);MasoudBridge.logoutDone();}})();";
        webView.evaluateJavascript(js,null);
    }

    private void injectLogin(){
        if(loginInProgress)return; loginInProgress=true;
        String js="(async function(){const sleep=ms=>new Promise(r=>setTimeout(r,ms));const txt=s=>(s||'').replace(/\\s+/g,' ').trim();const vis=e=>e&&e.offsetParent!==null;const setv=(x,v)=>{let d=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value');if(d&&d.set)d.set.call(x,v);else x.value=v;x.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:v}));x.dispatchEvent(new Event('change',{bubbles:true}));x.dispatchEvent(new Event('blur',{bubbles:true}));};try{let op=null;for(let tries=0;tries<40&&!op;tries++){let direct=[...document.querySelectorAll('a,button,[role=button]')].filter(vis);op=direct.filter(e=>/ورود/.test(txt(e.innerText))&&(/عضویت|ثبت نام|ثبت‌نام/.test(txt(e.innerText))||txt(e.innerText)==='ورود')).sort((a,b)=>txt(a.innerText).length-txt(b.innerText).length)[0]||document.querySelector('a[href*=login],a[href*=signin],button[aria-label*=ورود],[title*=ورود]');if(!op)await sleep(50);}if(!op){MasoudBridge.log('دکمه ورود/عضویت پیدا نشد');MasoudBridge.loginMissing();return;}op.click();let ph=null,pw=null;for(let i=0;i<60&&(!ph||!pw);i++){let ins=[...document.querySelectorAll('input')].filter(vis);pw=ins.find(x=>x.type==='password'||/کلمه عبور|رمز عبور|رمز/.test((x.placeholder||'')+' '+(x.name||'')+' '+(x.getAttribute('aria-label')||'')));ph=ins.find(x=>x!==pw&&(x.type==='tel'||/شماره همراه|موبایل|تلفن/.test((x.placeholder||'')+' '+(x.name||'')+' '+(x.getAttribute('aria-label')||''))))||ins.find(x=>x!==pw&&x.type!=='hidden'&&x.type!=='submit'&&x.type!=='button');if(!ph||!pw)await sleep(50);}if(!ph||!pw){MasoudBridge.log('فیلد شماره موبایل یا رمز پیدا نشد');MasoudBridge.loginMissing();return;}let pv="+q(phone.getText().toString().trim())+",wv="+q(password.getText().toString())+";ph.focus();setv(ph,pv);await sleep(100);pw.focus();setv(pw,wv);await sleep(100);if(String(ph.value).replace(/\\s/g,'')!==pv.replace(/\\s/g,'')||String(pw.value)!==wv){MasoudBridge.log('شماره موبایل یا رمز داخل فرم ثبت نشد');MasoudBridge.loginMissing();return;}MasoudBridge.log('شماره موبایل و رمز داخل فرم رجا ثبت شد');let bs=[...document.querySelectorAll('button,[role=button],input[type=submit]')].filter(vis);let b=bs.filter(x=>txt(x.innerText||x.value)==='ورود').sort((a,b)=>txt(a.innerText||a.value).length-txt(b.innerText||b.value).length)[0]||bs.find(x=>txt(x.innerText||x.value).includes('ورود'));if(!b){MasoudBridge.log('دکمه تأیید ورود پیدا نشد');MasoudBridge.loginMissing();return;}b.click();for(let i=0;i<60;i++){await sleep(50);let formVisible=vis(ph)||vis(pw);let body=txt(document.body.innerText);let account=/حساب کاربری|پروفایل|خروج از حساب/.test(body);if(account||!formVisible){MasoudBridge.loginClicked();return;}}MasoudBridge.log('ورود واقعی حساب رجا تأیید نشد');MasoudBridge.loginMissing();}catch(e){MasoudBridge.log('خطای ورود: '+e);MasoudBridge.loginMissing();}})();";
        webView.evaluateJavascript(js,null);
    }
    ...
    private void injectSearchFormV11(){
        ...
        String js="(async function(){"
            ...
            +"let pb=await wait(()=>document.querySelector('#dropdownPassenger')||[...document.querySelectorAll('button,[role=button],div')].filter(vis).filter(x=>/مسافر/.test(text(x))).sort((a,b)=>text(a).length-text(b).length)[0]);if(!pb){fail('کادر تعداد مسافر در رجا پیدا نشد');return;}pb.click();await sleep(100);"
            +"async function setCount(label,want,def){let row=await wait(()=>{let cs=[...document.querySelectorAll('div,li')].filter(vis).filter(x=>text(x).includes(label));return cs.sort((a,b)=>text(a).length-text(b).length).find(x=>x.querySelectorAll('button').length>=2)||null;});if(!row)return false;let buttons=[...row.querySelectorAll('button')].filter(vis),plus=buttons.find(x=>/\\+|افزایش/.test(text(x)+(x.getAttribute('aria-label')||'')))||buttons[buttons.length-1],minus=buttons.find(x=>/−|-|کاهش/.test(text(x)+(x.getAttribute('aria-label')||'')))||buttons[0];let nums=(text(row).match(/[0-9۰-۹]+/g)||[]).map(n=>Number(n.replace(/[۰-۹]/g,d=>'۰۱۲۳۴۵۶۷۸۹'.indexOf(d))));let cur=nums.find(n=>n>=0&&n<=6);if(cur===undefined)cur=def;while(cur<want&&plus){plus.click();cur++;await sleep(70);}while(cur>want&&minus){minus.click();cur--;await sleep(70);}return cur===want;}"
            +"let adultOk=await setCount('بزرگسال',"+adultCount+",1),childOk=await setCount('خردسال',"+childCount+",0);if(!childOk)childOk=await setCount('کودک',"+childCount+",0);if(!adultOk||!childOk){fail('تنظیم تعداد بزرگسال یا کودک تأیید نشد');return;}let confirm=[...document.querySelectorAll('button,[role=button]')].filter(vis).find(x=>text(x)==='تأیید'||text(x)==='تایید');if(confirm){confirm.click();await sleep(120);}else{pb.click();await sleep(100);}MasoudBridge.log('تعداد بزرگسال و کودک در رجا تأیید شد');"
            +"let wanted="+q(passengerTypeLabel())+";let typeControl=await wait(()=>{let exact=[...document.querySelectorAll('ng-select,.ng-select-container,[role=combobox],button,div,span')].filter(vis).filter(x=>['مسافران عادی','ویژه برادران','ویژه خواهران'].includes(text(x))).sort((a,b)=>text(a).length-text(b).length)[0];return exact&&(exact.closest('ng-select,[role=combobox],button,.ng-select-container')||exact);});if(!typeControl){fail('کادر نوع مسافر در رجا پیدا نشد');return;}typeControl.click();let choice=await wait(()=>[...document.querySelectorAll('.ng-option,[role=option],li,button,div,span')].filter(vis).filter(x=>text(x)===wanted).sort((a,b)=>text(a).length-text(b).length)[0]);if(!choice){fail('گزینه نوع مسافر پیدا نشد: '+wanted);return;}(choice.closest('.ng-option,[role=option],li,button')||choice).click();await sleep(120);let selected=[...document.querySelectorAll('ng-select,.ng-select-container,[role=combobox],button,div,span')].filter(vis).some(x=>text(x)===wanted);if(!selected){fail('انتخاب نوع مسافر تأیید نشد: '+wanted);return;}MasoudBridge.log('نوع مسافر در رجا تأیید شد: '+wanted);"
    ...
    private class JsBridge {
        ...
        @JavascriptInterface public void loginClicked(){ runOnUiThread(()->{ loginInProgress=false; loggedIn=true; loginRetryCount=0; log("ورود واقعی حساب رجا تأیید شد؛ انتقال به مرحله بعد"); handler.postDelayed(()->advanceAutomation(),100); }); }
